		...
		// CPlayerMove::RunPostThink
		{
			...
			auto post_think = [ ]( C_CSPlayer* player ) -> int {
				...
				if ( player->is_alive( ) /* index 155 for vfunc */ || *reinterpret_cast< std::uint32_t* >( reinterpret_cast< std::uint32_t >( player ) + 0x3A81 ) ) {
					...
					static auto post_think_v_physics = reinterpret_cast< bool( __thiscall* )( C_BaseEntity* ) >( n_utilities::pattern_scan( "client_panorama.dll", "55 8B EC 83 E4 F8 81 EC ? ? ? ? 53 8B D9 56 57 83 BB" ) );
					post_think_v_physics( player );
				}

				static auto simulate_player_simulated_entities = reinterpret_cast< bool( __thiscall* )( C_BaseEntity* ) >( n_utilities::pattern_scan( "client_panorama.dll", "56 8B F1 57 8B BE ? ? ? ? 83 EF 01 78 72" ) );
